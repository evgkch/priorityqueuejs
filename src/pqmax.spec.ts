import { default as PQMAX } from "./pqmax";

type ID = string;

describe('Tests for PQMAX:', () => {

    const q = new PQMAX<ID>();
    ...
    test('Extract the entities above', () => {
        const res: string[] = [];
        while (q.size > 0) {
            res.push(q.extract_max().entity);
        }
        expect(res).toEqual(['Mary', 'Alice', 'Bob', 'John']);
    ...
    test('Extract the entities above', () => {
        ...
        expect(res).toEqual(['John', 'Alice', 'Bob', 'Mary']);
        expect(q.size).toBe(0);
    });
    ...
    test('Trying extract the entities above more than the queue contain', () => {
        ...
        expect(res).toEqual(['John', 'Alice', 'Bob', 'Mary', undefined, undefined]);
        expect(q.size).toBe(0);
    });
    ...
    test('Remove the second entity from the queue and extract remains', () => {
        ...
        expect(res).toEqual(['Bob', 'John', 'Alice']);
        expect(q.size).toBe(0);
    });
});
